...
export const uploadSchema = z
    .object({
        ...
        stored_file_name: z.string().openapi({ example: "1234567890_assignment.pdf" }),
        file_size: z.number().openapi({ example: 1024 }),
        file_type: z.string().openapi({ example: "application/pdf" }),
        s3_url: z.string().openapi({
            example: "https://s3.amazonaws.com/bucket/1234567890_assignment.pdf",
        }),
...
export const getUploadsResponseSchema = z.array(uploadSchema).openapi({ ref: "GetUploadsResponse" });
