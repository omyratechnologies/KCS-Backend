...
export const coursesListResponseSchema = z
    .object({
        success: z.boolean(),
        data: z.object({
...
export const courseAnalyticsResponseSchema = z
    .object({
        success: z.boolean(),
        data: z.object({
            ...
            time_series_data: z.object({
                daily_enrollments: z.array(
                    z.object({
                        date: z.string(),
                        count: z.number(),
                    })
                ),
                weekly_engagement: z.array(
                    z.object({
                        week: z.string(),
...
export const updateSectionOrderRequestBodySchema = z
    .object({
...
export const updateLectureOrderRequestBodySchema = z
    .object({
        lecture_orders: z.array(
            z.object({
                id: z.string(),
                lecture_order: z.number(),
            })
...
export const createCourseAnnouncementRequestBodySchema = z
    .object({
        ...
        content: z.string().min(1).max(2000).openapi({ 
            example: "The next lecture will be moved to Friday at 2 PM.",
            description: "Announcement content/message"
        }),
        type: z.enum(["general", "urgent", "reminder", "assignment", "exam"]).default("general").openapi({
            description: "Type of announcement affecting priority and display"
        }),
        priority: z.enum(["low", "normal", "high", "urgent"]).default("normal").openapi({
            description: "Priority level for push notifications"
        }),
        visibility: z.enum(["all", "students", "instructors"]).default("all").openapi({
        ...
        expires_at: z
            ...
            .openapi({ 
                ...
                description: "When announcement expires and becomes inactive"
            }),
        send_push: z.boolean().default(true).openapi({
            description: "Whether to send push notification to users"
        }),
        send_email: z.boolean().default(false).openapi({
            description: "Whether to send email notification (future feature)"
        }),
    })
    .openapi({ ref: "createCourseAnnouncementRequestBody" });

export const createCampusAnnouncementRequestBodySchema = z
    .object({
        title: z.string().min(1).max(200).openapi({ 
            example: "Campus Holiday Notice",
            description: "Announcement title"
        }),
        content: z.string().min(1).max(2000).openapi({ 
            example: "Campus will be closed for maintenance on Friday.",
            description: "Announcement content/message"
        }),
        type: z.enum(["general", "urgent", "reminder", "event", "holiday"]).default("general").openapi({
            description: "Type of campus announcement"
        }),
        priority: z.enum(["low", "normal", "high", "urgent"]).default("normal").openapi({
            description: "Priority level for push notifications"
        }),
        target_audience: z.enum(["all", "students", "teachers", "staff"]).default("all").openapi({
        ...
        expires_at: z
            ...
            .openapi({ 
                example: "2024-12-31T23:59:59Z",
                description: "When announcement expires"
            }),
        send_push: z.boolean().default(true).openapi({
            description: "Whether to send push notification"
        }),
    })
    .openapi({ ref: "createCampusAnnouncementRequestBody" });

export const updateCourseAnnouncementRequestBodySchema = z
    .object({
        title: z.string().min(1).max(200).optional(),
        content: z.string().min(1).max(2000).optional(),
        type: z.enum(["general", "urgent", "reminder", "assignment", "exam"]).optional(),
        priority: z.enum(["low", "normal", "high", "urgent"]).optional(),
        visibility: z.enum(["all", "students", "instructors"]).optional(),
        expires_at: z.string().datetime().optional(),
        is_active: z.boolean().optional().openapi({
            description: "Whether announcement is active/visible"
        }),
    })
    .openapi({ ref: "updateCourseAnnouncementRequestBody" });

export const courseAnnouncementResponseSchema = z
    .object({
        success: z.boolean(),
        data: z.object({
            announcement: z.object({
                ...
                title: z.string(),
                content: z.string(),
                type: z.enum(["general", "urgent", "reminder", "assignment", "exam"]),
                priority: z.enum(["low", "normal", "high", "urgent"]),
                visibility: z.enum(["all", "students", "instructors"]),
                course_id: z.string(),
                created_by: z.string(),
    ...
    .openapi({ ref: "courseAnnouncementResponse" });

export const courseAnnouncementsListResponseSchema = z
    .object({
        success: z.boolean(),
        data: z.object({
    ...
    .openapi({ ref: "courseAnnouncementsListResponse" });
