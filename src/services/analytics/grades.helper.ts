...
export class GradesAnalyticsHelper {
                ...
                record.record_data.forEach((termData: any) => {
                    if (termData.marks && Array.isArray(termData.marks)) {
                        ...
                        const averagePercentage = Math.round(
                            (termData.marks.reduce((sum: number, mark: any) => 
                                sum + ((mark.mark_gained / mark.total_marks) * 100), 0
    ...
    private static async calculateSubjectWiseGrades(
        ...
        return Array.from(subjectGrades.values()).map((subject) => ({
            ...
            averagePercentage: subject.totalMaxMarks > 0 
                ? Math.round((subject.totalMarks / subject.totalMaxMarks) * 100) 
                : 0,
    ...
    private static calculateTermWiseAverages(records: any[]): TermAverage[] {
        ...
        records.forEach(record => {
            if (record.record_data && Array.isArray(record.record_data)) {
                record.record_data.forEach((termData: any) => {
                    if (termData.marks && Array.isArray(termData.marks)) {
                        const average = termData.marks.reduce((sum: number, mark: any) => 
                            sum + ((mark.mark_gained / mark.total_marks) * 100), 0
                        ) / termData.marks.length;
     ...
     */
    private static calculateSubjectTrend(grades: SubjectGradeEntry[]): "improving" | "declining" | "stable" | "insufficient_data" {
