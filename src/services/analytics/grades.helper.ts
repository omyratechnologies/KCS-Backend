...
export class GradesAnalyticsHelper {
     ...
     */
    static async getGradesData(user_id: string, campus_id: string): Promise<GradesData> {
        try {
            ...
            const currentTerm = await this.getCurrentTermGrades(records, campus_id);

            // Get all terms
            const allTerms = await this.getAllTermsGrades(records, campus_id);

            // Calculate subject-wise performance
            const subjectWise = await this.calculateSubjectWiseGrades(records, campus_id);
     ...
     * Get current term grades
     */
    private static async getCurrentTermGrades(records: any[], campus_id: string): Promise<CurrentTermGrade[]> {
        if (records.length === 0) {
            return [];
        }
        ...
        if (!latestRecord.record_data || !Array.isArray(latestRecord.record_data)) {
            return [];
        }

        // Get the latest term from the record
        const currentTerm = latestRecord.record_data.at(-1);
        if (!currentTerm.marks || !Array.isArray(currentTerm.marks)) {
            return [];
        }

        // Get subject names
        const subjectIds = currentTerm.marks.map((mark: any) => mark.subject_id);
     ...
     */
    private static async getAllTermsGrades(records: any[], campus_id: string): Promise<TermGradesData[]> {
        ...
        for (const record of records) {
            if (record.record_data && Array.isArray(record.record_data)) {
                record.record_data.forEach((termData: any) => {
                    if (termData.marks && Array.isArray(termData.marks)) {
                        const subjects: TermSubjectGrade[] = termData.marks.map((mark: any) => ({
                            subjectId: mark.subject_id,
                            subjectName: subjectNames[mark.subject_id] || "Unknown Subject",
                            marksGained: mark.mark_gained,
                            totalMarks: mark.total_marks,
                        ...
                        const averagePercentage = Math.round(
                            termData.marks.reduce(
                                (sum: number, mark: any) => sum + (mark.mark_gained / mark.total_marks) * 100,
                                0
     ...
     */
    private static async calculateSubjectWiseGrades(records: any[], campus_id: string): Promise<SubjectWiseGrades[]> {
        ...
        return [...subjectGrades.values()].map((subject) => ({
            ...
            averagePercentage:
                subject.totalMaxMarks > 0 ? Math.round((subject.totalMarks / subject.totalMaxMarks) * 100) : 0,
     ...
     * Identify improvement areas
     */
    private static identifyImprovementAreas(subjectWiseGrades: SubjectWiseGrades[]): ImprovementArea[] {
    ...
    private static calculateSubjectTrend(
        ...
        const earlyAvg = earlyGrades.reduce((sum, g) => sum + g.percentage, 0) / earlyGrades.length;
     ...
     */
    private static generateImprovementSuggestions(subject: SubjectWiseGrades): string[] {
     ...
     */
    static calculateGPA(grades: { percentage: number; creditHours?: number }[]): number {
        if (grades.length === 0) {
            return 0;
        }

        const totalCredits = grades.reduce((sum, grade) => sum + (grade.creditHours || 1), 0);
