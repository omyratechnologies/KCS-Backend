...
export class PaymentSecurityMonitor {
     ...
     */
    static getAuditLogs(campus_id?: string, limit: number = 100): PaymentAuditLog[] {
    ...
    static getSecurityMetrics(campus_id?: string): {
        ...
        return {
            ...
            failed_authentications: events.filter(
                (e) => e.event_type === "authentication_failure" || e.event_type === "authorization_failure"
            ).length,
            successful_payments: logs.filter((l) => l.event_type === "payment_verified" && l.success).length,
            failed_payments: logs.filter((l) => l.event_type === "payment_verified" && !l.success).length,
     ...
     */
    static getRecentSecurityEvents(campus_id?: string, limit: number = 100): SecurityEvent[] {
        let events = this.securityEvents;
    ...
    static getSecurityEventById(request_id: string): SecurityEvent | null {
        return this.securityEvents.find((event) => event.request_id === request_id) || null;
    }
