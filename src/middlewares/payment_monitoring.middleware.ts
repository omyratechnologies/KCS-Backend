...
export const paymentMonitoringMiddleware = () => {
    return async (ctx: Context, next: Next) => {
        ...
        console.info(
            `[PAYMENT_REQUEST] ${requestContext.method} ${requestContext.endpoint}`,
            {
        ...
        try {
            ...
            console.info(
                `[PAYMENT_RESPONSE] ${requestContext.method} ${requestContext.endpoint} - SUCCESS`,
                {
                    request_id: requestContext.request_id,
                    execution_time_ms: executionTime,
            ...
            console.error(
                `[PAYMENT_ERROR] ${requestContext.method} ${requestContext.endpoint}`,
                {
                    request_id: requestContext.request_id,
                    error_code: paymentError.code,
                    error_message: paymentError.message,
...
export const handlePaymentError = (
    ...
    const { error: paymentError, httpStatus } = PaymentErrorHandler.handleError(
        error,
        context
    );
    ...
    if (
        ["CRED", "AUTH", "SYS"].some((prefix) =>
            paymentError.code.startsWith(prefix)
        )
    ) {
        PaymentSecurityMonitor.logSecurityEvent({
            event_type: "suspicious_activity",
